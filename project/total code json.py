...
def del_csv():
    df = pd.read_csv('../project/celeba-dataset/list_eval_partition.csv')
    df = df.drop([df.index[-1]])
    df.to_csv('../project/celeba-dataset/list_eval_partition.csv', index=False)
...
for r in r10:
    ...
    for i in img:
        ...
        with urlopen(imgUrl) as f:
            with open('../testing/imageset/' + str(rank) + '_' +  ### 경로 수정 ###
                      r.replace('+', '') + str(n) + '.jpg', mode='wb') as h:  # w - write b - binary
                img = f.read()
                h.write(img)
                with open(os.path.join("../project/celeba-dataset/", 'list_eval_partition.csv'), newline='', mode='a') as file:
                    wr = csv.writer(file)
                    files = sorted(glob.glob(os.path.join("../testing/imageset/", '*.jpg')), key=os.path.getctime,
                                   reverse=True)
                    wr.writerow([files[0][20:], 2])


            src = cv2.imread(str(f"../testing/imageset/{str(rank)}_{r.replace('+', '')}{str(n)}.jpg"),
                             cv2.IMREAD_COLOR)
            print(f"../testing/imageset/{str(rank)}_{r.replace('+', '')}{str(n)}.jpg")
            resizing = cv2.resize(src, dsize=(178, 218), interpolation=cv2.INTER_CUBIC)
            gtorgb = cv2.cvtColor(resizing, cv2.COLOR_BGR2RGB)
            blue_mosters = imageio.imwrite(f"../testing/imageset/{str(rank)}_{r.replace('+', '')}{str(n)}.jpg",
                                           im=gtorgb, pilmode='CMYK', as_gray=False)

            facing_67("../testing/imageset/", rank)  ### 경로 수정 ###
